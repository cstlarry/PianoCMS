...
public class CallBackReport extends Application {
    ...
    private Parent createContent() {
        ...
        printBtn.setOnAction(e -> {
            ...
            String filename = "CallBackReport.txt";
            try {
    ...
    public void start(Stage stage) {
        ...
        Scene scene = new Scene(createContent(), 850, 400);
    ...
    private void setup() {
        ui = new ProjectUI("Enter Call Back Date");
        LocalDate today = LocalDate.now();
        LocalDate firstDay = today.with(TemporalAdjusters.firstDayOfMonth());
        String formattedDate = firstDay.format(formatter);
        ui.setField(0, formattedDate);
        appTitle = "Call Back Report";
        check.setSelected(true);
    } // end setup()

    public void run() {
        // your code goes here
        LocalDate callDate = ui.getFieldAsDate(0);
        ArrayList<String> callList = null;
        try {
            CustomerDAO.updateLastService();
            callList = CustomerDAO.getCallBackList(callDate);
        } catch(Exception e) {
            outputln(e);
        }

        outputln(callList.get(0));
        outputln();

        for(int i = 1; i < callList.size(); i++ ) {
            outputln(callList.get(i));
        }
